  ...
  return (
    <header
      className={cx(
        ...
        'px-3 py-2', //좌우,상하 패딩
        'backdrop-blur-sm bg-black/60', // 60% 검정 반투명 배경 + 뒤쪽을 약간 블러 처리
        'border-b' //하단 테두리
      )}
    >
      {/* 타이틀 가로로 정렬하고 간격 유지 */}
      ...
      <div className="flex items-center gap-3">
        ...
        <nav className="flex gap-4 items-center text-sm">
          <Link href="/login"><a className="hover:underline">로그인/회원가입</a></Link>
          <Link href="/reservations"><a className="hover:underline">예매내역조회</a></Link>
        </nav>
      </div>
    </header>
  );
